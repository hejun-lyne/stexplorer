...
```ts
stockDims   = { volume: 30, rsi: 0,  money: 30 }  // 权重（合计 100；RSI 为 0）
...
4. **个股表现评分（量能30 + 资金30；RSI 仅计算与展示，不计入加权）**：逐项分数/满分 + 进度条 + 说明行
...
| 个股 | `stockDims` | `{volume:30, rsi:0, money:30}` | 子项权重（RSI 为 0：只计算与展示，不计入） |
