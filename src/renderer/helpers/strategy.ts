...
async function rsiStrategyBacktest(startDate: string,
  ...
  //  d. 获取关联板块的资金流是否满足条件（如连续3天净流入且最后一天净流入超过一定阈值）
  //  e. 获取关联板块的历史K线数据，过滤到截止日期前的数据，并缓存下来以便重复使用
  //  g. 评估该股票的最近一段时间K线数据是否强于关联板块的K线数据（如RSI指标持续高于关联板块的RSI指标）(需要考虑是否纳入概念板块评估)
  //  i. 检查该股票k线数据是否处于上升趋势（如20日均线持续上升且股价在20日均线上方）
  //  k. 判断该股票的成交活跃度是否足够（如日均成交金额超过一定阈值），如果不满足条件，则跳过该股票；
  //。j. 根据关联板块资金流+股票k线与板块k线相对强弱指标+股票上升趋势指标+股票成交活跃度计算综合评分，如果评分超过一定阈值，则继续下一步，否则跳过该股票
  //  l. 使用RSI6, RSI12, RSI24进行RSI策略回测(使用optimizeRSIStrategy)，得到最佳策略并得到策略匹配评分，记录下来结果，将该股票纳入候选列表（注意对应的最佳策略参数也记录下来用于后续买卖点判断）
  //3. 对候选列表中的股票进行排序，选出评分最高的前N只股票纳入观察列表
  //4. 遍历观察列表，进行买卖点判断：
  //  a. 对于每只股票，获取其历史K线数据，过滤到截止日期前的K线数据
  //  b. 使用之前测算得到的最优策略进行买卖点判断, 如果没有买卖点和持仓，且加入观察列表超过5个交易日，则剔除出观察列表；如果有买卖点，则继续下一步判断
  //  c. 如果有买入点且当前没有持仓，且当前持仓不超过最大持仓数，则以当天收盘价执行买入:
  //     i. 计算当前总资金的10%， 如果当前可用资金小于总资金的10%，则不执行买入，否则执行买入（按最大可买入数量执行，需注意以100股为单位），记录买入价格和数量，更新可用资金和持仓数量
  //  d. 如果有卖出点且当前有持仓，则以当天收盘价执行卖出:
  //     i. 卖出全部持仓，记录卖出价格和数量，更新可用资金和持仓数量
  //  e. 如果没有买卖点，但是有持仓，则进行以下判断：
  //     i. 股票持仓只进行了一次买入操作，并且距离买入不超过3个交易日，并且当天收盘价低于买入当日k线实体的上线但是高于实体的中线，则执行一次加仓操作：
  //         1. 计算当前总资金的10%，如果当前可用资金小于总资金的10%，则不执行加仓，否则执行加仓（按最大可买入数量执行，需注意以100股为单位），记录加仓价格和数量，更新可用资金和持仓数量
  //  f. 进行止损判断：如果当天收盘价低于第一次买入当日k线实体的下线，则执行止损卖出操作：
  //5. 到回测周期结束，计算回测结果指标，如总收益率、年化收益率、最大回撤率、胜率等，并输出回测结果
}
