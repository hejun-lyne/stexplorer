...
const NetValueChart: React.FC<NetValueChartProps> = ({ chartOption, result, initialCapital }) => {
  ...
  useEffect(() => {
    if (chartOptionRef.current && result) {
      ...
      chart?.setOption(updated, true);
    }
  }, [darkMode, result, initialCapital, chart]);
...
const Backtest: React.FC<BacktestProps> = ({ onOpenStock, active }) => {
    ...
    const startBacktest = useCallback(async (resumeSnapshot?: BacktestEngine.BacktestSnapshot) => {
      ...
      const backtestParams = {
        ...
        macdParams: {
          ...
          aboveZeroOptions: macdRequireAboveZero ? [true] : [false],
          belowZeroOptions: macdRequireBelowZero ? [true] : [false],
        ...
        if (strategyType === 'rsi') {
          ...
          backtestResult = await strategy.run(
            ...
            },
            {
              onShouldCancel: () => cancelledRef.current,
              onShouldPause: () => pausedRef.current,
              onPendingOrdersReview: (orders, today) => new Promise((resolve) => {
                setPendingOrders(orders);
                setPendingOrderDecisions({});
                setCurrentBacktestDate(today);
    ...
    const marketCapDistributionColumns = [
        { title: '市值区间', dataIndex: 'marketCapRange', key: 'marketCapRange', width: 120 },
        { title: '交易次数', dataIndex: 'count', key: 'count', width: 90 },
        { title: '涉及股票数', dataIndex: 'uniqueCount', key: 'uniqueCount', width: 100 },
        { title: '盈利次数', dataIndex: 'winTrades', key: 'winTrades', width: 90 },
        { title: '亏损次数', dataIndex: 'lossTrades', key: 'lossTrades', width: 90 },
    ...
    return (
        <div className={styles.content}>
          ...
          <div className={styles.chartwrapper} style={{ flexDirection: 'column' }}>
            ...
            {pendingOrderReviewVisible && (
              <div style={{ padding: '0 16px 16px' }}>
                <Card
                  title={
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                      <span>待执行订单确认 ({pendingOrders.length} 笔)</span>
                      <Space size="small">
            ...
            {result && (
              <div style={{ padding: 16 }}>
                <NetValueChart chartOption={chartOption} result={result} initialCapital={initialCapital} />
                ...
                {result.marketCapDistribution && result.marketCapDistribution.length > 0 && (
                  <Card title="📊 总市值与胜率关系" size="small" style={{ marginBottom: 16 }}>
                    <Table 
                      columns={marketCapDistributionColumns}
                      dataSource={result.marketCapDistribution.map((d: any, i: number) => ({ ...d, key: i }))}
                      pagination={false}
                      size="small"
                    />
                  </Card>
