...
const StockDetail: React.FC<StockDetailProps> = ({ secid, active, name, firstQSAppear, backtestDate, onChangeUpdate, onOpenStock, onOpenUrl }) => {
  ...
  // 推送回调里读最新行情用 ref：tDetail 变化时不能重建订阅，否则会形成
  // 「订阅 → 推送 → setDetails → effect 重跑 → 重订阅 → 又推送」的死循环（UI 卡死）
  const tDetailRef = useRef(tDetail);
  tDetailRef.current = tDetail;

  useEffect(() => {
    if (!tDetailRef.current.zx || !tDetailRef.current.name) {
      runGetDetail();
    }
    if (!hasConfig) {
      const handlePushDetail = (data: any) => {
        // 闭包不再捕获 tDetail，改读 ref，避免订阅随行情更新重建
        const tDetail = tDetailRef.current;
        if (data) {
          let changed = false;
              ...
              >
                ...
                <Tabs defaultActiveKey={'news'} className={styles.rightTab} style={{ width: initWidth }}>
                  ...
                  <Tabs.TabPane tab={<span style={{ padding: '0 20px' }}>交易数据</span>} key={'coretrade'}>
                    <CoreTrade code={nDetails.code} klines={klines || undefined} />
                  </Tabs.TabPane>
