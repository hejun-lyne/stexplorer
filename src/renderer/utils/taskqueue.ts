...
export class SequentialTaskQueue {
  ...
  constructor(options?: SequentialTaskQueueOptions) {
    const opts = options || {};
    this.defaultTimeout = opts.timeout || 30;
    this.name = opts.name || 'SequentialTaskQueue';
    this.scheduler = opts.scheduler || SequentialTaskQueue.defaultScheduler;
  }
  ...
  private doneTask(task: TaskEntry, error?: any) {
    ...
    } else if (task.cancellationToken.cancelled) task.reject.call(undefined, task.cancellationToken.reason);
