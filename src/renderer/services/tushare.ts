  ...
  try {
    ...
    if (days) {
      return {
        ...
        avg_cost_1d: result.avg_cost_1d,
        avg_cost_3d: result.avg_cost_3d,
        avg_cost_5d: result.avg_cost_5d,
        avg_cost_10d: result.avg_cost_10d,
        avg_cost_20d: result.avg_cost_20d,
...
export async function MainInFilterStocksFromTushare(
  ...
  try {
    ...
    if (result.error) {
      console.error('[主力建仓] 服务端分析失败:', result.error);
      return { results: [], count: 0, passed_count: 0 };
    }
    return result as { results: MainInStockResult[]; count: number; passed_count: number };
  } catch (error) {
    logError(error, 'MainInFilterStocksFromTushare', '主力建仓分析失败');
    return { results: [], count: 0, passed_count: 0 };
  }
}
